...
/*--------------------------------------------------------------------------------------------
Writing on sLCD (value) on specified position (digit) + dot !!!!!!
--------------------------------------------------------------------------------------------*/
void slcdSetDot(uint8_t value, uint8_t digit){	

	switch(value){
		case 0x00:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_D | LCD_S_E | LCD_S_F); /* To display '0' we must active segments: d, e & f on first front plane */
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_A | LCD_S_B | LCD_S_C | LCD_S_DEC); /* To display '0' we must active segments: a, b & c on second front plane */
			break;
		case 0x01:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_C);  //TASK 2.5 - replace ZZZ with segment definition
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_B | LCD_S_C | LCD_S_DEC);  //TASK 2.5 - replace ZZZ with segment definition
			break;
		case 0x02:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_D | LCD_S_E | LCD_S_G );  //TASK 2.5 - replace ZZZ with segment definition
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_A | LCD_S_B | LCD_S_DEC);  //TASK 2.5 - replace ZZZ with segment definition
			break;
		case 0x03:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_G | LCD_S_D);  //TASK 2.5 - replace ZZZ with segment definition
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_A | LCD_S_B | LCD_S_C | LCD_S_DEC);  //TASK 2.5 - replace ZZZ with segment definition
			break;
		case 0x04:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_G | LCD_S_F);
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_B | LCD_S_C | LCD_S_DEC);
			break;
		case 0x05:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_G | LCD_S_F | LCD_S_D);
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_A | LCD_S_C | LCD_S_DEC);
			break;
		case 0x06:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_G | LCD_S_E | LCD_S_D  | LCD_S_F);
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_A | LCD_S_C | LCD_S_DEC);
		...
		case 0x07:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_C);
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_A | LCD_S_B | LCD_S_C | LCD_S_DEC);
			break;
		case 0x08:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_G | LCD_S_E | LCD_S_D  | LCD_S_F);
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_A | LCD_S_B | LCD_S_C | LCD_S_DEC);
		...
		case 0x0B:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_G | LCD_S_E | LCD_S_D | LCD_S_F);
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_C | LCD_S_DEC);
		break;
		case 0x0C:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_F | LCD_S_E | LCD_S_D);
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_A | LCD_S_DEC);
			break;
		case 0x0D:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_G | LCD_S_E | LCD_S_D);
			LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] = (LCD_S_B | LCD_S_C | LCD_S_DEC);
...
void slcdSetDP(uint8_t DP){  //selecting DP tp bo display
	if(DP > 3){slcdErr(4);}
	else{
		switch(DP){
			case 1:
...
void slcdDisplay(uint16_t value ,uint16_t format){	
	...
	tab[3] = value%format;
	tab[2] = (uint16_t)(value%(format*format))/format;
	tab[1] = (uint16_t)(value%(format*format*format)/(format*format));
	tab[0] = (uint16_t)(value%(format*format*format*format)/(format*format*format));
...
/*----------------------------------------------------------------------------
  Function for conversion purpose, displaying (value) in specified (format) with or without dots 
	Dots value: without following 0 !! 1000 - DP1;100 - DP2;10 - DP3;1 - COL
 *----------------------------------------------------------------------------*/
void slcdDisplayC(uint16_t value, uint16_t format ,uint16_t dots){	
	uint16_t one = (dots/1000)%10;
	uint16_t two = (dots/100)%10;
	uint16_t three = (dots/10)%10;
	uint16_t four = dots%10;
	...
	if(one < 2){
		if(two < 2){
			if(three < 2){
				if(four < 2){
					if(one == 1){
						slcdSetDot((uint16_t)(value%(format*format*format*format)/(format*format*format)),1);
					}
					else{
						slcdSet((uint16_t)(value%(format*format*format*format)/(format*format*format)),1);
					}
					if(two == 1){
						slcdSetDot((uint16_t)(value%(format*format*format)/(format*format)),2);
					}
					else{
						slcdSet((uint16_t)(value%(format*format*format)/(format*format)),2);
					}
					if(three == 1){
						slcdSetDot((uint16_t)(value%(format*format))/format,3);
					}
					else{
						slcdSet((uint16_t)(value%(format*format))/format,3);
					}	
					if(four == 1){
						slcdSetDot(value%format,4);
					}
					else{
						slcdSet(value%format,4);
					}
				}
				else{slcdErr(1);}
			}
